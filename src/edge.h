...
//     - one_way       : Si es falso, significa que tambien existe una arista en el grafo de 'dest'
//     a 'src'.
//                       Caso contrario, significa que solo existe la arista actual de 'src' a
//                       'dest'
...
struct Edge {
    ...
    static void parse_csv(const std::string& edges_path,
        ...
        while (true) {
            ...
            Edge* edge = new Edge(src_node, dest_node, std::stoi(max_speed), std::stod(length),
                                  std::strcmp(oneway, "True") == 0, std::stoi(lanes));
